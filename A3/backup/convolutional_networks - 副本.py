...
class DeepConvNet(object):
  ...
  def __init__(self,
    ...
    # 则maxpool相当于收缩一倍 (F,H,W)->(F,H/2,W/2)
    C, H, W = input_dims
    # max_pools = set(max_pools)  # 加速in查询

    for layer, F in enumerate(num_filters):
      self.params['W{}'.format(layer+1)] = weight_scale * torch.randn(F,C,3,3,dtype=dtype,device=device)
      self.params['b{}'.format(layer+1)] = torch.zeros(F,dtype=dtype,device=device)
      C = F  # C为上一层的Channel
    
    L = self.num_layers
    shrink = 4 ** len(set(max_pools))  # 计算缩减比例，每一个pool都要在宽高上缩减2倍，所以是4倍
    self.params['W{}'.format(L)] = weight_scale * torch.randn(C*H*W//shrink,num_classes,dtype=dtype,device=device)
    self.params['b{}'.format(L)] = torch.zeros(num_classes,dtype=dtype,device=device)        
  ...
  def loss(self, X, y=None):
    ...
    out, cache_dict[L] = Linear.forward(out,self.params['W'+str(L)],self.params['b'+str(L)])
    scores = out
    #####################################################
    #                 END OF YOUR CODE                  #
    #####################################################
    ...
    dout, dw, db = Linear.backward(dout,cache_dict[L])
    grads['W'+str(L)], grads['b'+str(L)] = dw + 2 * self.reg * self.params['W'+str(L)], db
    ...
    for layer in range(1,L)[::-1]:
      ...
      grads['W'+str(layer)], grads['b'+str(layer)] = dw + 2 * self.reg * self.params['W'+str(layer)], db
