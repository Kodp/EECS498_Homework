...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i, train_img in enumerate(x_train):
        for j, test_img in enumerate(x_test):
            ...
            test_img_flat = test_img.view(-1)
            dists[i, j] = ((train_img_flat - test_img_flat) ** 2).sum()  # !这里错的，没加开方，下面都没加开方
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i, train_img in enumerate(x_train):
        train_img_flat = train_img.view(-1)
         # 展成num_test行
        dists[i] = ((train_img_flat - test_flat) ** 2).sum(dim = 1)  
        # train_img_flat自动广播成num_test行，减去test_flat得到与所有样本的差值，然后自乘方，sum列压缩（一张图片是横着放的，列压缩sum了所有像素上的距离之和）
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # train_flat = x_train.view(num_train, 1, -1)             #  n * 1 * -1   
    # test_flat = x_test.view(num_test, -1)                   #      m * -1 
    # # dists = ((train_flat - test_flat) ** 2)               #  n * m * -1  尾->前扩散对齐
    # dists = ((train_flat - test_flat) ** 2).sum(dim = 2)    #  n * m       消去最后一维
    
    dists = ((x_train.view(num_train, 1, -1)  - x_test.view(num_test, -1) ) ** 2).sum(dim = 2)    
