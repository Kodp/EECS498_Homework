...
class ReLU(object):
    ...
    def backward(dout, cache):
        ...
        # x_01 = torch.where(x > 0, torch.ones_like(x), torch.zeros_like(x))  # 条件满足取第一个，否则取第二个
        #  x_01 = x.gt(0)  # gt Computes input>other element-wise. bool类型，乘法计算和乘0，1一样
        # dx = dout * x.gt(0)  # 没必要做乘法
        dx = dout.clone()
        dx[x < 0] = 0
        #####################################################
        #                  END OF YOUR CODE                 #
...
class TwoLayerNet(object):
    ...
    # @ 这是不用函数手动计算loss的函数，主要用于回顾自学，同时还发现了A2作业中的一个问题
    def loss_manual(self, X, y=None):

        # 这个的效率比用函数实现的略高，没有函数参数传递等开销
        """
        ...
        a_out, cache_LR_1 = Linear_ReLU.forward(
            X, self.params['W1'], self.params['b1'])

        scores, cache_L_2 = Linear.forward(
            a_out, self.params['W2'], self.params['b2'])
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        # 这个bug找了好久，其中一个原因是这个bug只会导致网络的性能比正确的实现仅仅低3~5个百分点左右，实际上还是能到50%正确率，你可以自己训练一下。我觉得一般人都不会注意到这个问题，或者不把他视为一个问题，但实际上任何细微的变化都应该被察觉发现解决。如果这个东西写在框架里了并且实现了一些应用，之后再查只会难度指数增加
        ...
        # $ a_out == 0可能存在隐患，虽然relu之后有0，因为在实际操作中，计算机可能无法准确地表示浮点数，从而导致误差。虽然这种方法在某些情况下可能有效，但为了避免潜在问题，最好使用ReLU激活函数的输入值来计算梯度，即y=xw+b。
...
class FullyConnectedNet(object):
        ...
        for i in range(cnt_hidden):
            ...
            if i == 0:
                self.params['W'+idx] = weight_scale * \
                    torch.randn(input_dim, hidden_dims[i], dtype=dtype, device=device)
            else:
                self.params['W'+idx] = weight_scale * \
                torch.randn(hidden_dims[i - 1], hidden_dims[i], dtype=dtype, device=device)
            self.params['b'+idx] = weight_scale * \
                torch.zeros(hidden_dims[i], dtype=dtype, device=device)

        self.params['W'+str(cnt_hidden+1)] = weight_scale*torch.randn(
            hidden_dims[-1], num_classes, dtype=dtype, device=device)
        self.params['b'+str(cnt_hidden+1)] = weight_scale * \
            torch.zeros(num_classes, dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        ##################################################################
        # 第一次做是可以不加dropout的，后面会让修改成有dropout的
        ...
        for i in range(1, cnt_layer):
            ...
            if self.use_dropout:
                ...
                last_out, d_cache = Dropout.forward(last_out, self.dropout_param)
        ...
        # if mode == 'train' and self.use_dropout:# 第一次
        # if self.use_dropout:
        ...
        #     if i != cnt_layers - 1:
        #         out, cache = Linear_ReLU.forward(xIn, w, b)
        #         if p is not None:
        #             mask = torch.rand(out.size) < p  # rand,01均匀分布
        #             out *= mask
        #             mask_cache[i + 1] = mask
        #     else:
        #         out, cache = Linear.forward(xIn, w, b)
        #     fcRelu_cache[i+1] = cache
        #     xIn = out
        # scores = xIn
        #################################################################
        #                      END OF YOUR CODE                         #
        ...
        # does not includes a factor of 0.5 to simplify the expression for  #
        # the gradient.                                                     #
        #####################################################################
        # Replace "pass" statement with your code
        ...
        grads['W{}'.format(cnt_layer)] = dw + 2 * self.reg * \
            self.params['W{}'.format(cnt_layer)]
        ...
        for i in range(cnt_layer - 1, 0, -1):
            ...
            grads['W{}'.format(i)] = dw + 2 * self.reg * \
                self.params['W'+str(i)]
            grads['b{}'.format(i)] = db

        # loss, dS = softmax_loss(scores, y)  # 第一次
        # reg_loss = 0
        # for i in range(cnt_layers):
        #     reg_loss += self.reg * (self.params['W'+str(i+1)] ** 2).sum()
        # loss += reg_loss
        # for i in range(cnt_layers, 0, -1):
        #     if i == cnt_layers:
        #         dout, dw, db = Linear.backward(dout=dS, cache=fcRelu_cache[i])
        #     else:
        #         dout, dw, db = Linear_ReLU.backward(dout=dS, cache=fcRelu_cache[i])
...
def create_solver_instance(data_dict, dtype, device):
    ...
    solver = Solver(model=model, data=data_dict, optim_config={
        "learning_rate": 0.1
    },
        lr_decay=0.93, num_epochs=30, print_every=500, device=device)
...
def rmsprop(w, dw, config=None):
    ...
    next_w = w  # in-place,ipynb里说了可以这么做
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            mask = (torch.rand(x.shape, device=x.device) > p) / \
                (1 - p)  # 这里p是指丢掉的输出比例，所以大于p保留，除以1-p保持缩放
            out = x * mask
            ##############################################################
            #                   END OF YOUR CODE                         #
            ##############################################################
