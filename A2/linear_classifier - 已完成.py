...
def svm_loss_vectorized(  # @第2个版本，全部向量化，代码清晰
    ...
    margins[margins < 0] = 0 
    # print(margins.shape)
    L1 = margins.sum() / N - 1 # loss值的第一项， -1是由于每一张图片里的yi的margin值为1，加起来就导致每张图片都加了1，除N之后减掉1即可。
    ...
    grad_mgs = margins
    grad_mgs[torch.arange(grad_mgs.shape[0]),y] = 0 # 标签处改0
    grad_mgs[grad_mgs > 0] = 1 # 有值的地方改1，矩阵乘法中直接计算+1 * X[i]
    cnt_row_1 = torch.sum(grad_mgs, dim = 1)  # countnonzoro返回的类型是int，不匹配grad_mgs, 用sum免去用to（值已经都是01了）
    # !用了to autograd可能出问题
    grad_mgs[torch.arange(grad_mgs.shape[0]),y] = -cnt_row_1
    # print("grad_mgs:",grad_mgs)
    dW = grad_mgs.T.mm(X).T / N
    # print("dW current:", dW)
