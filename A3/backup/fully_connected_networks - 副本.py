...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        a_out, a_cache = Linear_ReLU.forward(
        ...
        sum_row_exp_scores = scores.exp().sum(dim=1, keepdim=True)  # (1,C)

        # Sk - logSUM, e^{Sk-logSUM} = e^Sk/SUM, 先减法就避免除法
        log_probs = scores - sum_row_exp_scores.log()
        probs = log_probs.exp()
        ...
        _, relu_cache = a_cache

        grad_S = probs
        grad_S[torch.arange(N), y] -= 1
        grad_S /= N

        grads['b2'] = grad_S.sum(dim=0)
        grads['W2'] = relu_cache.T.mm(grad_S) + 2 * self.reg * self.params['W2']

        # grad_h0[h1 < 0.000001] = 0  #
        # h1[h1 > 0] = 1
        # grad_h0 = grad_h1 * h1
        grad_h0 = grad_S.mm(self.params['W2'].T)  # 
        grad_h0[grad_h0 < 0] = 0 
...
def create_solver_instance(data_dict, dtype, device):
    ...
    solver = Solver(model=model,data=data_dict, optim_config={
                    'learning_rate': 1e-3,
                  },lr_decay=0.95, batch_size=128,num_epochs=10,print_every=500,device=device)
    ##############################################################
    #                    END OF YOUR CODE                        #
    ##############################################################
    return solver
