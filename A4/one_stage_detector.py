...
class FCOSPredictionNetwork(nn.Module):
    """
    接受FPN特征，预测三种输出: 预测框、分类、中心性(bounding boxes, class ID, centerness)
    ...
    def __init__(
        ...
        # 创建两个stem网络，用于处理回归和中心性预测
        # 使用in_channels和stem_channels创建层。
        # 每个卷积层的权重初始化为均值为0，标准差为0.01的正态分布，偏置全部初始化为0。
        # 卷积stride=1、padding=0、保证输入特征的尺寸不变。因为要在特征图的每个位置进行预测，不丢失任何位置的信息，
        ######################################################################
        # Fill these.
        stem_cls = []
        stem_box = []
        ...
        # logit是未归一化的原始预测值
        # 这里是在说，咱这个网络只输出原始预测值，后面的sigmoid处理不放在这个类里面
        ######################################################################
        # Replace these lines with your code, keep variable names unchanged.
        # Class prediction conv

        self.pred_cls = nn.Conv2d(c, num_classes, 3, padding=1)
        ...
        # 负偏置的主要目标是控制模型预测所有类别（包括背景类别）的置信度，使其不会过于乐观。
        # 具体来说，这个负偏置使得所有类别的初始预测概率接近于1/100 (1/(1+e^-(-ln(99)))，
        # 这意味着模型开始的预测比较谨慎、不偏向于任何一个类别，对于训练的稳定性和性能有所帮助。

        # 真正的原因是，目标检测任务中存在大量的负样本（或称为背景）。如果模型在开始训练时过于乐观地预测出现目标，这可能会导致训练不稳定。
        # 设置这个初始偏置的目的是使模型对预测目标存在的可能性保持谨慎态度。

        # 此外，设置偏置的具体值-math.log(99)，这个值的大小是可能是通过实验选取的，用以提供一个良好的训练起点。
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        # 原论文中box回归和centerness是分别从两个不同的stem层处理得到的，但在这里的实现中，我们让他们都从同一个stem层获取。
        # 这是因为box回归和centerness都涉及到位置相关的信息，他们之间的关联性更强。这也是最近的一些相关论文通常采用的做法。
        ######################################################################
        # Fill these with keys: {"p3", "p4", "p5"}, same as input dictionary.
        class_logits = {}
        boxreg_deltas = {}  # LTRB delta
        centerness_logits = {}
...
def fcos_match_locations_to_gt(
    ...
    """
    在FCOS中，每个FPN层级的每个特征图位置都会进行预测，因此需要与一个合适的目标框进行监督学习。
    这个函数的主要目的是在不同的FPN（Feature Pyramid Network）层级中为每个特征图位置匹配最合适的目标框（Ground Truth box）。
    ...
    for level_name, centers in locations_per_fpn_level.items():
        ...
        stride = strides_per_fpn_level[level_name]  # (H*W, 1)

        # 将stride解压(升维)，拆成x坐标和y坐标
        #      (H*W,2)==>(H*W,2,1)==>(H*W,1),(H*W,1)
        x, y = centers.unsqueeze(dim=2).unbind(dim=1)
        # unsqueeze把维度抬升
        # unbind把一个维度拆成多个tensor(移除这个维度)

        # 提升gt_boxes的维度并分解x0, y0, x1, y1四个坐标
        x0, y0, x1, y1 = gt_boxes[:, :4].unsqueeze(dim=0).unbind(dim=2)

        # 计算每个中心点到每个GT框的距离(应该>0)，保存到pairwise_dist
        pairwise_dist = torch.stack([x - x0, y - y0, x1 - x, y1 - y], dim=2)

        # Pairwise distance between every feature center and GT box edges:
        # shape: (num_gt_boxes, num_centers_this_level, 4)
        # 调整pairwise_dist的维度，让最后一个维度代表距离值
        pairwise_dist = pairwise_dist.permute(1, 0, 2)

        # 使用原始FCOS的锚点匹配规则：锚点必须在GT框内部，即
        ...
        # 使用最大距离更新match_matrix，只保留在上下边界值内的点
        ...
        # 在匹配质量不高（<1e-5）的情况下，将匹配索引设为-1，表示未匹配
...
def fcos_get_deltas_from_locations(
    ...
    deltas /= stride  # normalized by FPN stride.
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    centerness = None
    # Replace "pass" statement with your code
    val1 = torch.min(deltas[:, 0], deltas[:, 2])  # l, r
    val2 = torch.min(deltas[:, 1], deltas[:, 3])  # top, bottom
...
class FCOS(nn.Module):
    """
    FCOS: Fully-Convolutional One-Stage Detector 完全使用卷积的单阶段检测器

    包含全部的FCOS结构:backbone，FPN，预测层(Head)。
    可在训练时计算loss，推理时预测框子。
    ...
    def forward(
        ...
        for box in matched_gt_boxes:
            ...
            matched_gt_deltas.append(
                {'p3': delta_p3, 'p4': delta_p4, 'p5': delta_p5})
        ...
        center_gt = fcos_make_centerness_targets(
            matched_gt_deltas.reshape(-1, 4)).reshape(B, WH, 1)
        gt_cls_onehot = torch.zeros((N * WH, C + 1), device=pred_cls_logits.device)
        gt_cls_onehot[torch.arange(
            N * WH), matched_gt_boxes[:, :, -1].reshape(-1).long()] = 1
        label = gt_cls_onehot[:, :-1].reshape(N, WH, -1)
        loss_cls = sigmoid_focal_loss(pred_cls_logits, label.float())
        loss_box = 0.25 * \
                   F.smooth_l1_loss(pred_boxreg_deltas,
                                    matched_gt_deltas, reduction="none")
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            level_pred_scores = level_pred_scores[torch.arange(
                level_pred_scores.shape[0]), level_pred_classes]

            # Step 2:
            # Replace "pass" statement with your code
            level_pred_classes = level_pred_classes[level_pred_scores >
                                                    test_score_thresh]
            ...
            level_pred_boxes = level_pred_boxes[level_pred_scores >
                                                test_score_thresh]
            level_pred_scores = level_pred_scores[level_pred_scores >
                                                  test_score_thresh]
