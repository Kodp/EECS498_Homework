...
def count_tensor_elements(x: Tensor) -> int:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # 计算元素个数，乘起来 shape是torch.Size类型，可遍历
    num_elements = 1
    for i in x.shape:
        num_elements *= i
...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # 返回一个张量，包含从start到stop中10的倍数
    st = start + (10 - start % 10)
    ed = stop + 1
    print(st, ed)
    if st <= ed:
        x = torch.arange(st, ed, 10, dtype = torch.float64)
    else:
        x = torch.tensor([], dtype=torch.float64) # (0,)
...
def slice_indexing_practice(x: Tensor) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
    ...
    #建立要求的tensor，每个tensor一行代码解决
    last_row = x[-1]
    third_col = x[:, 2:3]  # 同一维度，用2:3而不是2
    first_two_rows_three_cols = x[:2, :3]
    even_rows_odd_cols = x[::2,1::2]  # 偶数行奇数列
...
def reverse_rows(x: Tensor) -> Tensor:
    ...
    # idx = torch.arange(x.shape[0] - 1, -1, -1)  # 倒序range，依然是左闭右开，不包含-1
    # # idx = range(x.shape[0] - 1, -1, -1)
    # y = x[idx]
    y = x[range(x.shape[0])[::-1]]  # [::-1]倒序
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    comprehensions). # comprehensions是推导式
    ...
    Returns:
        y: Tensor of shape (N, C) and where C = 1 + max(x) is one more than the
            max value in x.（List是类别数组，所以最大类别号+1就是列数） 
            The nth row of y is a one-hot-vector representation
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    y = torch.zeros(len(x), 1 + max(x), dtype = torch.float32)  # 生成（N，C）0矩阵
    y[torch.arange(y.shape[0]), x] = 1  # 每一行上，X位置的值为1，正好是标签的位置
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def sum_positive_entries(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # 不用循环（包括推导式）、不修改源数据、一次比较、一次索引，解决求和大于0的元素
    pos_sum = torch.sum(x[x > 0]).item()  # item()是为了取值
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    idx = t.permute(1, 0, 2).reshape(3, 8)
    y = x[idx]  # x是一行，直接用张量做索引，让他变成索引的shape
    """
    
    # y = x.view(-1,3,4)
    # y = torch.cat((y[0],y[1]), dim = 1)
    
    y = x.view(-1,3,4).transpose(1,2).reshape(-1,3).T
    #print(y)
    #y= y.reshape(8,3).T
    #print(y)
    # 两张， 然后转置， 然后合并成8行3列， 之后转置
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # 不修改原数据
    y = x.clone()
    idx = x.argmin(dim = 1)  # 消除了列，得到了每行最大值的位置（argmin)
    y[torch.arange(x.shape[0]), idx] = 0 # 每一行，位置处，置为0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    # @非常好的向量化计算例子
    mu_col = x.sum(dim = 0) / x.shape[0]  # 求和除以行数, 得到一个行向量

    sum_x_minus_mu_2 = torch.sum((x - mu_col) ** 2, dim = 0)  # 广播，逐行相减，自乘方，求和, 得到行向量

    sigma_col = torch.sqrt(sum_x_minus_mu_2 / (x.shape[0] - 1)) # 无偏+开方
    
    y = (x - mu_col) / sigma_col  # 广播，逐行减除；
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
