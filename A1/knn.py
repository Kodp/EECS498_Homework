...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    dists = x_train.new_zeros(num_train, num_test) #  .new_zeros() create tensors the same type but possibly different shapes
    ##########################################################################
    ...
    for i, train_img in enumerate(x_train):
        for j, test_img in enumerate(x_test):
            dists[i, j] = ((train_img- test_img) ** 2).sum() ** 0.5
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i, train_img in enumerate(x_train):
        # train_img_flat = train_img.view(-1)
         # 展成num_test行
        dists[i] = ((train_img - x_test) ** 2).sum(dim = (1, 2, 3)) ** (1 / 2)
        """
        (train_img - x_test) ** 2 得到 m个差值平方图像，每个图像是三通道，所以按照通道、行、列每个值都要sum起来，就是sum(dim=(1,2,3))，得到m个数据，然后sqrt
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # Replace "pass" statement with your code
    """
    train_flat = x_train.reshape(num_train, 1, -1)             #  n * 1 * -1   
    test_flat = x_test.reshape(num_test, -1)                   #      m * -1 
    # dists = ((train_flat - test_flat) ** 2)               #  n * m * -1  尾->前扩散对齐
    dists = ((train_flat - test_flat) ** 2).sum(dim = 2) ** (1/2)    #  n * m       消去最后一维
    # 速度很慢
    #!Using this, torch will internally create an a x b x c array for broadcasting, then you may run into memory issues.
    """
    ...
    # 笔记本记了模拟
    A = x_train.view(num_train,-1)  # 展成num_train行, 一行一个样本
    B = x_test.view(num_test,-1)
    AB2 = A.mm(B.T) * 2  # 
    dists = ((A ** 2).sum(dim = 1).view(-1, 1) - AB2 + (B ** 2).sum(dim = 1).view(1, -1)) ** 0.5

    # dists = ((x_train.view(num_train, 1, -1)  - x_test.view(num_test, -1) ) ** 2).sum(dim = 2) ** (1 / 2)
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    选取K个最近样本，并找到这些样本中，标签出现次数最多的那个标签（编号最小）
    给出y_train的标签
    ...
    _, test2train_dist_min_pos = torch.topk(dists, k, dim = 0,largest = False)
    # 从小到大的topk 
    #$ 参考一下别人的
    test2train_dist_min_pos = test2train_dist_min_pos.T
    # assert test2train_dist_min_pos.shape == (num_test, k)  # numtest行，k列
    # print(test2train_dist_min_pos)
    
    label_value = y_train[test2train_dist_min_pos]
    
    # assert label_value.shape == test2train_dist_min_pos.shape
    
    # print(label_value)
    
    y_pred, _ = torch.mode(label_value, dim = 1)
...
class KnnClassifier:
    ...
    def predict(self, x_test: torch.Tensor, k: int = 1):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        # shape (num_test, C, H, W), num_test张图片，返回num_test个标签
        dists = compute_distances_no_loops(self.x_train, x_test)
        # print("NB!\n")
        y_test_pred = predict_labels(dists, self.y_train, k)
        
        # min_pos = torch.argmin(dists, dim = 1)  # 获得每行距离的位置，消除了列
        # y_test_pred = self.y_train[min_pos]  # 直接输入索引数组能够逐行取结果
...
def knn_cross_validate(
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x_train_folds, y_train_folds = list(x_train.chunk(num_folds)), list(y_train.chunk(num_folds))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    #! 要以当前的块为测试块，其他的是训练块，而不是另外划分最后一个块为测试块。ipynb前面给数据的时候test块已经划分好了
    for i, k in enumerate(k_choices):
        cur_train, cur_label = None, None
        accs = []
        for j in range(num_folds):  # 不用第j个分类
            
            cur_train = [x for k, x in enumerate(x_train_folds) if  k != j]
            cur_label = [x for k, x in enumerate(y_train_folds) if  k != j]
            
            cur_train = torch.cat(cur_train, dim = 0)  # list堆叠
            cur_label = torch.cat(cur_label, dim = 0)
            
            print(cur_train.shape)
            cf = KnnClassifier(cur_train, cur_label)
            accs.append(cf.check_accuracy(x_train_folds[j], y_train_folds[j], k))  # 第j个分类是测试块
    ...
    #             continue
    #         if cur_train == None:
    #             cur_train = x_train_folds[j].clone()
    #             cur_label = y_train_folds[j].clone()
    #         else:
    #             cur_train = torch.cat((cur_train, x_train_folds[j]), 0)
    #             cur_label = torch.cat((cur_label, y_train_folds[j]), 0)
    #     print(cur_train.shape, cur_label.shape)
    #     cf = KnnClassifier(cur_train, cur_label)
    #     # pd_res = cf.predict(x_train_folds[-1], k)
    #     accs = []
        
    #     for j in range()
        
    #     # for j in range(num_folds):
    #     #     acc = cf.check_accuracy(x_train_folds[j], y_train_folds[j], k)
    #     #     accs.append(acc)
    #     k_to_accuracies[k] = accs
