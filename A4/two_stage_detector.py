"""
docstring很重要
@特征图时，坐标计算上横向为x，纵向为y，和通常的顺序颠倒，down-top是上下长度
"""
import math
...
class RPNPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        cur_in_c = in_channels
        for c in stem_channels:
            conv = nn.Conv2d(cur_in_c, c, kernel_size=3, padding=1)
            nn.init.normal_(conv.weight, 0, 0.001)
        ...
        # Replace "pass" statement with your code
        # Faster RCNN用一个stem提取特征然后用两个小网络得到objectness和predbox
        # objectness在这里实现不是用两个值，而是一个值+sigmoid，所以这里通道为1
        self.pred_obj = nn.Conv2d(cur_in_c, 1 * num_anchors, kernel_size=1, padding=0)
        nn.init.normal_(self.pred_obj.weight, 0, 0.01)
        nn.init.zeros_(self.pred_obj.bias)
        # pred_box需要x1y1 x2y2一共四个值，所以通道为4
        self.pred_box = nn.Conv2d(cur_in_c, 4 * num_anchors, kernel_size=1, padding=0)
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for key, feat in feats_per_fpn_level.items():
            ...
            pred_obj = self.pred_obj(x)  # (N,H,W,num_anchors)
            object_logits[key] = pred_obj.reshape(pred_obj.shape[0], -1)
            pred_box = self.pred_box(x)
            boxreg_deltas[key] = pred_box.reshape(pred_box.shape[0], -1, 4)
...
def generate_fpn_anchors(
    ...
    """
    这个函数生成了FPN（特征金字塔网络）每个级别每个位置的多个anchor boxes(使用给定的aspect_ratios生成不同形态的anchor)。
    根据FPN级别的中心位置、步长、步长尺度和长宽比，生成不同形状和大小的anchor boxes。
    这些anchor boxes中心对应于给定的FPN位置，并且格式为XYXY。
    返回结果是一个字典，键为FPN级别（如"p3"，"p4"，"p5"），值为形状为(H * W * A, 4)的张量，其中A = len(aspect_ratios)，表示每个位置的anchor boxes数量。
    ...
    Args:
        locations_per_fpn_level: Centers at different levels of FPN (p3, p4, p5),
            ...
            (p3,p4,p5)不同特征图中每个点在原始图像中所属的中心位置(一小片区域都属于一个中心点)
            {
                "p3": (H*W,2),
                "p4": (H*W,2),
                "p5": (H*W,2),
        ...
        strides_per_fpn_level: Dictionary of same keys as above, each with an
            ...
            backbone之后，featmap对应于原图的stride，{"p3": 8, "p4": 16, "p5": 32}
        ...
        aspect_ratios: Anchor aspect ratios to consider at every location. We
            consider anchor area to be `(stride_scale * FPN level stride) ** 2` 方形面积=边长^2
            and set new width and height of anchors at every location:
                new_width = sqrt(area / aspect ratio)  变形为长方
    ...
    for level_name, locations in locations_per_fpn_level.items():
        # location: (H*W,2)
        level_stride = strides_per_fpn_level[level_name]  # int, 8/16/32

        # List of `A = len(aspect_ratios)` anchor boxes.
        anchor_boxes = []

        area = (stride_scale * level_stride) ** 2  # 放到上面比较好，避免重复计算
        ...
        for aspect_ratio in aspect_ratios:
            ...
            new_height = area / new_width
            # @ 这里是横向为x，纵向为y，和通常的顺序颠倒
            x1y1 = locations - torch.tensor([new_width / 2, new_height / 2])  # (HW,2)
            x2y2 = locations + torch.tensor([new_width / 2, new_height / 2])
            anchor_boxes.append(torch.cat((x1y1, x2y2), dim=1))  # (HW,2)cat(HW,2)->(HW,4)
            ##################################################################
            #                           END OF YOUR CODE                     #
            ##################################################################
        ...
        # permute或者tranpose之后变得不连续，用contiguous+view或者reshape
        # 正常创建的张量一般都是连续的，直接用view性能更好
...
@torch.no_grad()
def iou_slow(boxes1: torch.Tensor, boxes2: torch.Tensor) -> torch.Tensor:
    """
    原始版本IOU
    """
    ...
    for i in range(M):
        ...
        for j in range(N):
            b = boxes2[j]
            l, r = torch.max(a[0], b[0]), torch.min(a[2], b[2])
            t, d = torch.max(a[1], b[1]), torch.min(a[3], b[3])
            dx = max(r - l, 0)
            dy = max(d - t, 0)
            I = dx * dy
            S1 = (a[2] - a[0]) * (a[3] - a[1])
...
def iou(box1: torch.Tensor, box2: torch.Tensor) -> torch.Tensor:
    ...
    Args:
        box1: Tensor of shape `(M, 4)` giving a set of box co-ordinates.
        box2: Tensor of shape `(N, 4)` giving another set of box co-ordinates.
    ...
    ##########################################################################
    # 这里用的是 I/(S1+S2-I)模式，不是I/S(大矩形)模式
    # 结合ipynb示例食用
    a = box1[:, None, :]  # (M, 1, 4)    
    lt = torch.max(a[:, :, :2], box2[:, :2])  # (M, 1, 2)-(N, 2)->(M, N, 2) 取0，1
    rd = torch.min(a[:, :, 2:], box2[:, 2:])  # (M, 1, 2)-(N, 2)->(M, N, 2) 取2，3
    
    dxdy = (rd - lt).clamp(min=0)  # (M, N, 2)
    I = dxdy[:, :, 0] * dxdy[:, :, 1]
    S1 = (box1[:, 2] - box1[:, 0]) * (box1[:, 3] - box1[:, 1])  # (M,)
    S2 = (box2[:, 2] - box2[:, 0]) * (box2[:, 3] - box2[:, 1])  # (N,)
    # (M, 1)+(N, )->(M, N)
    iou = I / (S1[:, None] + S2 - I)  # (M, N)/(M, 1)+(N, )-(M, N, 1)
...
def rcnn_match_anchors_to_gt(
    ...
    """
    这个函数根据IoU阈值将锚框（或RPN proposal）匹配到一组真实框（GT boxes）。
    如果真实框不存在，所有锚框被匹配到背景。
    如果存在真实框，计算每个锚框与所有真实框的IoU，将IoU最大的真实框分配给每个锚框。
    对于匹配质量低于低阈值的锚框，将其设为背景（-1）；对于匹配质量在阈值之间的锚框，设为中性（-1e8）。
...
def rcnn_get_deltas_from_anchors(
    ...
    ph = (anchors[:, 3] - anchors[:, 1])  # (y2 - y1) / 2
    
    bx = (gt_boxes[:, 0] + gt_boxes[:, 2]) / 2  # (x1 + x2) / 2
    by = (gt_boxes[:, 1] + gt_boxes[:, 3]) / 2
    bw = (gt_boxes[:, 2] - gt_boxes[:, 0]) 
    bh = (gt_boxes[:, 3] - gt_boxes[:, 1])  # (y2 - y1) / 2

    tx = (bx - px) / pw
    ty = (by - py) / ph
    tw = torch.log(bw / pw)
    th = torch.log(bh / ph)  # (N, )
    deltas = torch.stack((tx, ty, tw, th), dim=1)  # (N, 4) stack升维堆叠
    deltas[gt_boxes[:, 0] == -1] = -1e8  # 取那些行，赋值为-1e8
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def rcnn_apply_deltas_to_anchors(
    ...
    Args:
        deltas: Tensor of shape `(N, 4)` giving box regression deltas txtytwth. 
    ...
    bh = ph * torch.exp(deltas[:, 3])  # (N,)
    # 也要XYXY
    output_boxes = torch.empty_like(anchors)
